setTimeout(function(){
    ...
    jQuery('button#ASCM_addall').click(function(){
        ...
        jQuery('div#MySubscribedItems div.itemChoice:not(.inCollection)').each(function(){
            items.push()
            var collections = {};
            var id = jQuery(this).attr('id').replace('choice_MySubscribedItems_','');
            collections[collection_id] = {add: true, title: collection_name};
            var data = {sessionID: window.g_sessionID, publishedfileid: id, collections: collections}
